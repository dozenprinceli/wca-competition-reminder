...
- 支持全部 17 个 WCA 官方项目。
- 每位收件人可独立设置项目、国家/地区、大洲和可选的距离筛选条件。
...
- 对用户及管理员操作记录脱敏审计日志，按级别分流并只保留最近 7 天。
...
- 可选：启用了 Maps JavaScript API 的 Google Maps Platform 浏览器 API key，用于地图选点
...
5. 如需在订阅台使用 Google 地图选点，在 `[web]` 中设置浏览器 API key。
...
TOML 收件人的经纬度也是可选项：必须同时填写或同时省略。设置正数
...
打开 `http://127.0.0.1:8080/`。注册时必须填写邮箱、称呼和 6 位邮箱验证码，并勾选同意接收 WCA 比赛通知邮件；经纬度可以同时留空。验证码 5 分钟失效，同一邮箱每 50 秒最多发送一次，浏览器按钮会倒计时 60 秒。邮箱是订阅的唯一 ID，查询、修改和取消均不使用令牌。修改时先输入邮箱，页面会载入现有的称呼、经纬度、最远距离、项目和地区设置；取消时只需邮箱。最远距离必须为正数，设置时经纬度必填。取消后，该邮箱尚未发送的排队通知也会停止。

配置 `[web].google_maps_api_key` 后，经纬度区域会提供 Google 地图选点弹窗；点击地图并确认即可自动回填 6 位小数坐标。需要在 Google Cloud 项目中启用 [Maps JavaScript API](https://developers.google.com/maps/documentation/javascript/cloud-setup)，并按部署域名设置 [HTTP referrer 限制和 API 限制](https://developers.google.com/maps/api-security-best-practices)。浏览器 API key 会发送到客户端，不应把它当作服务器端密钥。
...
`[[admins]]` 用户名和密码登录。控制台包含运行检查点、订阅用户、比赛和邮件投递数据；当前页面为只读，不会修改业务状态。登录成功后使用服务端生成的 HttpOnly、SameSite 会话 Cookie，会话 8 小时后过期，服务重启也会使其立即失效。公开部署时必须通过 HTTPS 访问，并将 `config.toml` 权限限制为仅运行账户可读。
...
schema v4 会在启动时使用事务自动迁移现有的 v3 SQLite 状态库。旧订阅的
`max_distance_km` 保持为 `NULL`，行为不变，无需删库或重新建立基线。v2 及更早版本的结构无法安全升级，仍会被拒绝。
...
项目、地域和距离筛选同时生效：只有所有已配置条件都匹配时，该收件人才会收到提醒。
...
Web 服务会记录验证码请求、注册、查询、修改、取消，以及管理员登录、退出和数据查看等审计事件。邮箱在日志中会被遮盖，密码和验证码不会写入日志。PM2 配置将自己的重复日志副本指向 `/dev/null`，避免绕过 7 天保留周期；生产排查请直接读取 `logs/run.*.log` 和 `logs/web.*.log`。
