...
打开 `http://127.0.0.1:8080/`。注册时必须填写邮箱、称呼和 6 位邮箱验证码，并勾选同意接收 WCA 比赛通知邮件。每个订阅可配置 1 至 10 条关注条件，每条分别包含经纬度、最远距离、项目和地区；条件内的已配置筛选同时满足才算命中，任意一条条件命中即推送。验证码 5 分钟失效，同一邮箱每 50 秒最多发送一次，浏览器按钮会倒计时 60 秒。邮箱是订阅的唯一 ID，查询、修改和取消均不使用令牌。修改时页面会载入完整条件列表；取消时只需邮箱。最远距离必须为正数，设置时该条件的经纬度必填。取消后，该邮箱尚未发送的排队通知也会停止。
...
schema v5 会在启动时使用事务自动迁移当前生产版本的 v4 SQLite 状态库。每条旧订阅的经纬度、最远距离、项目、国家/地区和大洲会全量复制为序号 0（界面中的条件 1），升级前后的匹配行为不变，无需删库或重新建立基线。v3 及更早版本的结构不再自动升级。
