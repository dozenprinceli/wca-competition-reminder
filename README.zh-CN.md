...
`web.base_url` 是浏览器订阅台的公网根地址，用于通知邮件底部的“修改订阅”和“取消订阅”链接。
本地开发默认使用 `http://127.0.0.1:8080`；生产环境应配置为用户可访问的 HTTPS 地址。如果订阅台
部署在域名下的子路径中，该路径也要包含在配置值中。地址不能包含用户名密码、查询参数或锚点。
...
订阅台支持通过 `?tab=modify&email=user%40example.com` 或
`?tab=cancel&email=user%40example.com` 直接切换到修改或取消页签并预填邮箱，同时兼容 `edit` 和
`unsubscribe` 两个页签别名。打开取消链接只会选择页签并填入邮箱，仍需用户主动提交后才会取消。
